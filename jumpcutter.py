...
# pip install pillow audiotsm scipy numpy pytube
...
def parse_args():
    ...
    parser.add_argument('--silent_threshold', type=float, default=0.04,
    ...
    parser.add_argument('--sounded_speed', type=float, default=1,
                        help="the speed that sounded (spoken) frames should be played at. Typically 1.")
    parser.add_argument('--silent_speed', type=float, default=7,
    ...
    parser.add_argument('--frame_quality', type=int, default=0,
...
frameRate = 10 or args.frame_rate
...
if len(os.listdir(TEMP_FOLDER)) < 3:  # When folder is not filled with frames.
    command = f"ffmpeg -thread_queue_size 8192 -threads {THREADS} -i '{INPUT_FILE}' -qscale:v {FRAME_QUALITY} '{TEMP_FOLDER}/frame%06d.jpg' -hide_banner"
...
if not os.path.isfile(audio_file_path):
    command = f"ffmpeg -thread_queue_size 8192 -threads {THREADS} -i '{INPUT_FILE}' -ab 160k -ac 2 -ar {SAMPLE_RATE} -vn '{audio_file_path}'"
    subprocess.call(command, shell=True)
...
command = f"ffmpeg -thread_queue_size 8192 -threads {THREADS} -i '{INPUT_FILE}' 2>&1"
...
for line in params:
    ...
    if m is not None:
        frameRate_ = float(m.group(1))
        frameRate = frameRate or frameRate_
        if abs(frameRate - frameRate_) > 0.01:
            print("HELLO\n\n"
                  "I AM HERE\n\n "
                  "LOOK AT ME \n\nhi")
            while (yn := input(f"FPS provided: {frameRate}, found: {frameRate_}.\n "
                               f"Use {frameRate_}? y/n: ")) not in "yn":
                pass
            if yn == "y":
                frameRate = frameRate_
        else:
            print(f"FPS provided: {frameRate}, found: {frameRate_}.")
            print("Framerates match.")
...
if maxAudioVolume == 0:
    with open(LOG_FOLDER, "a+", encoding="utf-8") as log_file:
        log_file.write(f"[WARNING]: maxAudioVolume = 0 \n")

    maxAudioVolume = np.nextafter(0, 1)
...
while True:
    ...
    try:
        ...
        if FUCK_ON_FAIL:
            raise e
        x = input("Audio file is too big :(, retry on input, enter 42 to skip file")
        try:
            if int(x) == 42:
                raise e
        except Exception:
...
# input(f"Framerate: {frameRate}")

command = f"ffmpeg -thread_queue_size 8192 -threads {THREADS} -framerate {frameRate} -i '{TEMP_FOLDER}/newFrame%06d.jpg' -i '{TEMP_FOLDER}/audioNew.wav' -strict -2 '{OUTPUT_FILE}'"
subprocess.call(command, shell=True)
